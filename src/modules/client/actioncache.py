...
together with a materialised table of the key attribute values for which
the installed actions conflict with one another.

It replaces the actions.stripped, actions.offsets and keys.conflicting
flat files that were formerly rebuilt in their entirety after every
image-modifying operation. The database is instead reconciled against
the installed package catalog, so that only the manifests of packages
which have actually changed need to be examined."""
...
# change; a mismatch causes a transparent rebuild.
SCHEMA_VERSION = 1
...
class ActionCache(object):
    ...
    def update(self, progtrack=None):
        ...
        try:
            ...
            # state is recomputed for exactly these keys.
            affected = {}
            cur = con.cursor()
    ...
    def selfcheck(self):
        ...
        validate incremental maintenance in the field."""
