...
installed catalog is always a subset of the known catalog whose entries
...
without materialising entire catalog parts in memory the way the JSON
catalog implementation must.

While this backend is being proven, the database is written alongside
the JSON catalogs and preferred for reading when it is current; the
no-image-state-db debug value opts out entirely. It is derived data
and can always be rebuilt from the JSON catalogs (and those in turn
from the publisher catalogs)."""
...
def summary_locales(cat):
    """Return the set of locales for which the given catalog has
    summary parts."""
...
def build_db(pathname, kcat, icat):
    ...
    catalog 'kcat' and the 'installed' catalog 'icat'.
...
class ImageCatalog(object):
    ...
    catalog part order (dependency data, then summary data by
    locale)."""
    ...
    def __open(self):
        ...
        image gains one on its first catalog rebuild)."""
        ...
        if uv != SCHEMA_VERSION:
            ...
            # empty catalog and the next rebuild replaces it.
            con.close()
            return None
        self.__con = con
        return con
    ...
    def last_modified(self):
        ...
        the known catalog this database was built from, or None."""
    ...
    def get_entry(self, pub=None, stem=None, ver=None, pfmri=None):
        """Return the base catalog entry for the given package (minus
    ...
    def entry_actions(
        ...
        tuples, where 'entry' is the base catalog entry. Mirrors
    ...
    def gen_actions(self, t, info_needed, excludes=EmptyI, locales=None):
        ...
        data for entries originating from v0 catalog sources is
...
class SolverCatalog(object):
    ...
    present in the catalog (only possible for packages installed from
    long-obsolete v0 catalog sources) yield no actions. Callers gate
...
class ListCatalog(object):
    ...
    # Callers reference these via catalog instances.
    ...
    def entry_actions(
        ...
        for t, entry, astrs in self.__db.entry_actions(
            ...
            if cb is not None and not cb(t, entry):
                continue
            if not astrs:
                # Entries from v0 catalog sources have no cached
...
class StateCatalog(object):
    ...
    from long-obsolete v0 catalog sources yield no cached actions."""
    ...
    @property
    def parts(self):
        """A dictionary-alike (name keys) of the catalog parts the
...
class _PartView(object):
    ...
    catalog part, for callers (notably the image catalog rebuild)
...
def _gen_lazy_actions(m, info_needed, locales, excludes):
    ...
    corresponding to the requested catalog information, mirroring
